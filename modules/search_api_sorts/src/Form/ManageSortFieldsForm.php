...
class ManageSortFieldsForm extends FormBase {
  ...
  public function buildForm(array $form, FormStateInterface $form_state, IndexInterface $search_api_index = NULL, $search_api_display = NULL) {
    ...
    if ($disabled = empty($this->index->status())) {
      drupal_set_message(t('Since the index for this display is at the moment disabled, no sorts can be activated.'), 'warning');
    }
  ...
  public function validateForm(array &$form, FormStateInterface $form_state) {
    foreach ($form_state->getValue('sorts') as $key => $v) {
      if ($v['status']) {
        if (!$v['label']) {
          $form_state->setErrorByName("sorts][$key][label", t("You can't set an empty label."));
        }
        elseif (strlen($v['label']) > 80) {
          $form_state->setErrorByName("sorts][$key][label", t('Labels cannot be longer than 80 characters, but "@label" is @count characters long.',
